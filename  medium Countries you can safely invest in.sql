...
(
	SELECT
		p.id,
		c.name
	FROM Person p join Country c on (substr(p.phone_number,1,3) = c.country_code)
), t2 as (
...
select 
	t1.name,
	avg(duration) over (partition by name) country_avg,
	avg(duration) over () global_average
from t1 join t2 on (t1.id = t2.caller_id)
)
select distinct name country from t3 where  country_avg > global_average;
